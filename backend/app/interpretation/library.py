...
# Initial Pre-seeded Georgian AI Draft Content Library (Hard Jester Voice)
INITIAL_GEORGIAN_DRAFTS: dict[str, str] = {
    "relationship.attraction.strong_chemistry.v1": (
        "ერთ ოთახში რომ შემოდიხართ, ჰაერი ისე მძიმდება, თითქოს ვიღაცამ კონდიციონერი გამორთო და დრამა ჩართო. ნაპერწკლები კარგია, მაგრამ ხანძარსაწინააღმდეგო სისტემა თუ არ მუშაობს, მალე ორივე ერთად დაიფერფლებით."
    ),
    "relationship.attraction.strong_chemistry.v2": (
        "თვალებით ფლირტს მორჩით — მთელმა ოთახმა გაიგო, რაც გინდათ, თქვენ კი ისევ ისე იქცევით, თითქოს შემთხვევით აღმოჩნდით ერთსა და იმავე კედელთან მიყუდებულები."
    ),
    "relationship.attraction.magnetic_chemistry.v1": (
        "ერთმანეთს ისე ეჯახებით, როგორც ორი დამუხტული სადენი — იცით, რომ საშიშია, მაგრამ მაინც ხელს ჰკიდებთ. ლოგიკა კარიდან გადის იმ წამსვე, როგორც კი ერთმანეთის სიახლოვეს აღმოჩნდებით."
    ),
    "relationship.harmony.emotional_resonance.v1": (
        "ერთმანეთის განწყობას უსიტყვოდ ხვდებით, მაგრამ ამას ხშირად იყენებთ იმისთვის, რომ საუბარს თავი აარიდოთ. ტელეპათიის იმედად ყოფნა საბოლოოდ ყოველთვის გაუგებრობით მთავრდება."
    ),
    "relationship.growth.complementary_balance.v1": (
        "სრულიად განსხვავებული რაკურსით უყურებთ სამყაროს. იდეალური ბალანსია ზუსტად მანამ, სანამ გადაწყვეტთ, ვინ მართავს მანქანას და ვისი პლეილისტი ჩაირთვება."
    ),
    "relationship.growth.dynamic_emotional_tension.v1": (
        "თქვენთან ამინდი უფრო ხშირად იცვლება, ვიდრე მთაში. დრამა ცხოვრებას კი აფერადებს, მაგრამ ყოველ საღამოს თეატრალურ წარმოდგენას ნერვები დიდხანს ვერ გაუძლებს."
    ),
    "relationship.harmony.core_harmony.v1": (
        "ფუნდამენტურ საკითხებში იმდენად ერთნაირად ფიქრობთ, რომ კამათის დაწყებაც კი მოსაწყენად გეჩვენებათ. ფრთხილად: ზედმეტი ერთსულოვნება აზროვნებას აზარმაცებს."
    ),
    "relationship.growth.contrasting_perspectives.v1": (
        "ერთი და იმავე ფაქტიდან ორი სრულიად საპირისპირო დასკვნა გამოგაქვთ და ორივე დარწმუნებული ხართ საკუთარ გენიალურობაში. სარკეში ყურება კარგია, ოღონდ თუ მარტო მეორის ნაკლს ამჩნევთ, სარკის გატეხვა მოგინდებათ."
    ),
    "relationship.growth.ego_friction.v1": (
        "ორი ძლიერი ეგო ერთ სივრცეში — ჰაერი იმდენად მწირია, რომ ყოველი გადაწყვეტილება ტახტისთვის ბრძოლად იქცევა. გვირგვინი ერთია და ორივე ერთად ვერ დაიხურავთ; რიგრიგობით ტარება ისწავლეთ."
    ),
    "relationship.attraction.warm_affection.v1": (
        "ისეთი სათუთი ზრუნვით ექცევით ერთმანეთს, თითქოს ფაიფურის ჭიქები იყოთ და არა ზრდასრული ადამიანები. ეს მყუდრო იდილია სასიამოვნოა, სანამ ერთმანეთის გაღიზიანებას ზედმეტი თავაზიანობით არ დაიწყებთ."
    ),
    "relationship.harmony.gentle_affinity.v1": (
        "ერთმანეთს ისეთი გადამეტებული სიფრთხილით ექცევით, თითქოს მყიფე მინის სათამაშოები გეჭიროთ ხელში. მზრუნველობა კარგია, მაგრამ საბავშვო ბაღის რეჟიმიდან გამოსვლაც დროა."
    ),
    "relationship.communication.intellectual_flow.v1": (
        "ისეთი სისწრაფით ცვლით აზრებსა და ირონიას, რომ გვერდით მყოფს თავი ნელთბილ სტატისტად ეჩვენება. ინტელექტუალური პინგ-პონგი ბრწყინვალეა, ოღონდ გრძნობებზე ლაპარაკის დროსაც სარკაზმს თუ არ გამორთავთ, მარტო დარჩებით."
    ),
    "relationship.communication.mutual_understanding.v1": (
        "საერთო ენა ისე მარტივად იპოვეთ, თითქოს ერთსა და იმავე კლასში იჯექით ბოლო მერხზე და მასწავლებელს დასცინოდით. ასეთი კომფორტული დიალოგი საშიშია: იმდენად ეჩვევით ერთმანეთის გაგებას, რომ გარესამყაროსთან კონტაქტი გეზარებათ."
    ),
    "relationship.growth.pacing_tension.v1": (
        "ერთს უკვე ჩემოდანი აქვს ჩალაგებული, მეორე კი ჯერ კიდევ ამინდის პროგნოზს აანალიზებს. სანამ ერთი ფიქრობს, მეორე უკვე შეცდომას უშვებს — სამაგიეროდ, ბალანსი დაცულია."
    ),
    "relationship.growth.dynamic_spark.v1": (
        "თქვენს შორის სიმშვიდე არ არსებობს: თუ პრობლემა არ არის, ხელოვნურად შექმნით, რომ მერე აზარტით განიხილოთ. მთავარია, კამათის შემდეგ ვახშამი მეგობრულად გაიყოთ და ჩანგლები იარაღად არ გამოიყენოთ."
    ),
    "relationship.attraction.energized_collaboration.v1": (
        "როცა ერთად საქმეს იწყებთ, ენერგია ისე გემატებათ, თითქოს მთელ სამყაროს უნდა მოუგოთ შეჯიბრი. ტემპი შთამბეჭდავია, სანამ გაირკვევა, რომ საწვავი გამოგელიათ და რუკა სახლში დაგრჩათ."
    ),
    "relationship.attraction.dynamic_drive.v1": (
        "ორივეს გაზის პედალი გიყვართ და მუხრუჭს სისუსტედ თვლით. სიჩქარე კარგია, მაგრამ საჭეს თუ ორივე ერთდროულად მოქაჩავთ, ბოძთან შეხვედრა გარდაუვალია."
    ),
    "relationship.stability.shared_optimism.v1": (
        "პრობლემებს ისეთი მსუბუქი იუმორით უყურებთ, თითქოს კრიზისი თქვენ კი არა, მეზობელს დაემართა. ოპტიმიზმი გადამდებია, მაგრამ ვარდისფერი სათვალე რეალურ ორმოებს ვერ ამოავსებს."
    ),
    "relationship.harmony.generous_affection.v1": (
        "ერთმანეთს კომპლიმენტებითა და მხარდაჭერით ისე ავსებთ, თითქოს პირადი პიარ-სააგენტოები გქონდეთ დაქირავებული. ეს მხარდაჭერა გულწრფელია, სანამ ერთ-ერთი თქვენგანი ჩრდილში არ აღმოჩნდება."
    ),
    "relationship.attraction.intense_magnetism.v1": (
        "ისეთი მზერით უყურებთ ერთმანეთს, თითქოს პაროლი იცით, რომლითაც მეორის ფსიქიკის გატეხვა შეიძლება. ეს მიზიდულობა კი არა, ფსიქოლოგიური დეტექტივია, სადაც ორივე ეჭვმიტანილიცაა და გამომძიებელიც."
    ),
    "relationship.stability.long_term_grounding.v1": (
        "დაპირებებს ისე სერიოზულად ეკიდებით, თითქოს ნოტარიუსთან იყოთ ხელმოწერილი. ციხესიმაგრე კი ააშენეთ, ოღონდ შიგნით სიცოცხლეც რომ დატოვოთ, კარგი იქნება."
    ),
    "relationship.notice.independent_dynamics.v1": (
        "პირად სივრცეს ისეთი ეჭვიანობით იცავთ, თითქოს ერთმანეთისგან თავის დაცვა გჭირდებოდეთ. თავისუფლება კარგია, მაგრამ თუ საერთო არაფერი დაგრჩათ, ურთიერთობას რატომღა არქმევთ?"
    ),
    "relationship.overall.exceptional_flow.v1": (
        "ყველაფერი ისე ბუნებრივად და უხარვეზოდ ლაგდება, რომ საეჭვოცაა — ცხოვრება ასეთი მარტივი არასდროსაა. ასეთი სინერგია იშვიათია, ოღონდ ილუზიაში ნუ ჩავარდებით, რომ ეს თავისით გაგრძელდება სამუდამოდ."
    ),
    "relationship.overall.balanced_synergy.v1": (
        "გაქვთ საერთო ბაზა, მაგრამ ინდივიდუალურ სივრცესაც კბილებით იცავთ — და სწორედ ეს გინარჩუნებთ ინტერესს. ბალანსი შესანიშნავია, მთავარია, ეს დისტანცია ემოციურ სიცივეში არ გადაიზარდოს."
    ),
    "relationship.overall.stimulating_friction.v1": (
        "თქვენი კავშირი მუდმივი პროვოკაციაა — არცერთი არ აძლევთ მეორეს მშვიდად ყოფნის უფლებას. მუდმივი გამოწვევა მატონიზირებელია, მაგრამ დასვენებაც რომ სჭირდება ადამიანს, ეგეც გაითვალისწინეთ."
    ),
    "relationship.overall.independent_paths.v1": (
        "ორი სრულიად დამოუკიდებელი სამყარო ხართ, რომლებიც ერთმანეთს დროდადრო გადაკვეთენ და მერე ისევ საკუთარ ტრაექტორიაზე ბრუნდებიან. თავისუფლება კარგია, ოღონდ თუ საერთო არაფერი დაგრჩათ, ურთიერთობას რატომღა არქმევთ?"
    ),
